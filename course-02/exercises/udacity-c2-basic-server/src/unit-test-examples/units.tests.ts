...
describe('concat', () => {
  ...
  it('should concatenate Ronald and Sulbaran', () => {
    const result = concat("Ronald", "Sulbaran");
    expect(result).to.equal("RonaldSulbaran");
  });

  it('should throw an error if one the strings is empty', () => {
    expect(()=>{ concat("Ronald", "") }).to.throw('empty string')
  });

  it('should throw an error if one the strings is empty', () => {
    expect(()=>{ concat("", "Sulbaran") }).to.throw('empty string')
  });
